...
class TestFundBatchEstimate:
    ...
    def funds(self):
        ...
        return [
            Fund.objects.create(
                fund_code="000001", fund_name="测试基金1", latest_nav=Decimal("1.5000")
            ),
            Fund.objects.create(
                fund_code="000002", fund_name="测试基金2", latest_nav=Decimal("2.0000")
            ),
        ]
    ...
    def test_batch_estimate_with_nonexistent_fund(self, client):
        """测试：基金不存在时，返回错误"""
        response = client.post(
            "/api/funds/batch_estimate/", {"fund_codes": ["999999"]}, format="json"
        )
...
class TestPositionOperationAPIWithRecalculation:
    ...
    def fund(self):
        return Fund.objects.create(
            fund_code="000001", fund_name="测试基金", latest_nav=Decimal("1.5000")
        )
