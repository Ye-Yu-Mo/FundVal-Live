...
class TestEastMoneyIsAvailable:
    ...
    def test_uses_fund_mn_finfo_endpoint(self):
        ...
        with patch("requests.get") as mock_get:
            ...
            assert "FundMNFInfo" in called_url, f"应该调 FundMNFInfo 端点, 实际: {called_url}"
            assert "000001" in called_url, f"应该查询 000001 测试基金, 实际: {called_url}"
...
class TestListAvailableSources:
    ...
    def test_excludes_danjuan(self):
        ...
        assert "danjuan" not in available, f"danjuan 应被过滤, 实际可用: {available}"
    ...
    def test_list_sources_unchanged(self):
        ...
        assert "danjuan" in all_sources, "list_sources() 应包含所有注册源（包括不可用的）"
    ...
    def test_each_available_source_has_is_available_true(self):
        ...
        for name in SourceRegistry.list_available_sources():
            ...
            assert source.is_available() is True, f"可用源 {name} 的 is_available() 应为 True"
