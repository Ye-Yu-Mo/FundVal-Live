...
- **系统版本卡片**：设置页面顶部展示后端版本、前端版本、数据库状态、系统状态，点击刷新可重新检测
- **健康检查版本号**：`/api/health/` 返回 `version` 字段
- **Footer 版本显示**：所有页面底部显示服务端版本号
- **登录页版本号**：登录页底部显示版本
- **个人信息卡片**：设置页面展示用户名、角色，提供修改密码和退出登录入口

### Fixed

- **Celery worker 未注册新任务**：更新部署流程确保 `celery-worker` 和 `celery-beat` 同步获取最新代码
- **日内估值时区修正**：`timezone.now()` → `timezone.localtime()`
