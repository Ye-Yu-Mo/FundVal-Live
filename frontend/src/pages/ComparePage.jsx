...
const ComparePage = () => {
  ...
  const buildRadarData = () => {
    ...
    return keys.map((k) => {
      ...
      funds.forEach((f) => {
        entry[f.fund_code] = f.returns?.[k] != null ? parseFloat(f.returns[k]) : 0;
      });
      return entry;
    });
  };
  ...
  const buildTableData = () => {
    ...
    metricKeys.forEach((k) => {
      ...
      funds.forEach((f) => {
        ...
        else if (['1m', '3m', '6m', '1y'].includes(k))
          val = f.returns?.[k] != null ? parseFloat(f.returns[k]) : null;
        else val = f.metrics?.[k] != null ? parseFloat(f.metrics[k]) : null;
        row[f.fund_code] = val;
        if (val != null) {
          if (['max_drawdown', 'volatility'].includes(k)) {
        ...
        return (
          <span
            style={{
              ...
              background: isBest ? '#f6ffed' : undefined,
