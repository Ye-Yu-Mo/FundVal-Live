...
- **实时估值**：双引擎架构 — akshare 全市场估值 + 穿透估算 fallback，通过基金持仓成分股权重 + 个股实时行情反向加权推算净值，优先覆盖 ETF/指数型基金
- **多数据源**：东方财富（主源）、养基宝（扫码登录）、小倍养基（手机号登录）、新浪（场内行情）、蛋卷（评级排名），SourceRegistry 注册表 + 健康检查机制
- **估值 fallback 链**：akshare → 穿透估算 → unavailable，每层独立超时和异常处理，估值源可追溯
...
- **持仓管理**：多账户、父子账户结构，支持买入/卖出流水，自动重算持仓，兼容仅含金额的隐私模式导入
- **估值可视化**：估值来源标识（akshare / 持仓推算）、收盘估值标记、持仓市值统一字段
